...
library(bnlearn)
library(Rgraphviz)
#data <- read.csv("C:/Users/Sophie/workspace/EvacModel/sample7.csv", header=TRUE)
...
X <- data[, c(c(9:14), 20, 21, 22, 28, 30)]
...
#res1 <- gs(X, alpha=0.05, blacklist = tiers2blacklist(list(c(demo, personal, house),  c(see, "EvacExpense", official),"Evac")))
res1 <- gs(X, alpha=0.05, blacklist = tiers2blacklist(list(house, c( see, official, "EvacExpense"),"Evac")))
