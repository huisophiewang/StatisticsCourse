...
blklist <- list(c(demo1), c(demo2, house, info_tv, info_social_media, evac_notice), c(risk, evac_ability), evac_decision)
...
learned_net <- hc(X, score="bic", blacklist=tiers2blacklist(blklist), debug=TRUE)
...
# model averaging
boot <- boot.strength(X, R = 1000, algorithm = "hc", algorithm.args = list(score="bic", blacklist=tiers2blacklist(blklist)))
